...
logger = logging.getLogger("d2_content_sync")
...
@dataclass
class SyncStats:
    supplier_id: int = D2_SUPPLIER_ID
...
async def sync_d2_content(limit: int = 0) -> Dict[str, Any]:
    stats = SyncStats()
    logger.info("Запуск D2 content sync для supplier_id=%s", D2_SUPPLIER_ID)

    async with get_async_db() as session:
        mapping_stmt = (
            select(CatalogSupplierMapping)
            .where(CatalogSupplierMapping.supplier_id == D2_SUPPLIER_ID)
        ...
        for mapping in mapping_rows:
            raw = (
                await session.execute(
                    select(RawSupplierFeedProduct).where(
                        RawSupplierFeedProduct.supplier_id == D2_SUPPLIER_ID,
            ...
            if raw is None:
                ...
                if stats.skipped_no_raw <= MAX_LOGGED_WARNINGS:
                    _warn(
                        stats,
                        "Не найдена raw D2 запись для content sync: sku=%s, supplier_code=%s",
                        mapping.sku,
            ...
            if not isinstance(payload, dict):
                ...
                if stats.skipped_no_content <= MAX_LOGGED_WARNINGS:
                    _warn(
                        stats,
                        "Неожиданная структура source_payload у D2: sku=%s, supplier_code=%s",
                        mapping.sku,
                        mapping.supplier_code,
                    )
                continue

            ua_description = _extract_payload_value(payload, "description_ua")
            ru_description = _extract_payload_value(payload, "description_ru")
            fallback_title = _normalize_string(raw.name_raw)
            ...
            if ua_description:
                ...
                await _upsert_content(
                    session=session,
                    sku=mapping.sku,
                    supplier_id=D2_SUPPLIER_ID,
                    language_code="ua",
                    title=_extract_payload_value(payload, "name_ua") or fallback_title,
            ...
            if ru_description:
                ...
                await _upsert_content(
                    ...
                    supplier_id=D2_SUPPLIER_ID,
                    language_code="ru",
                    title=_extract_payload_value(payload, "name_ru") or fallback_title,
            ...
            if not has_content:
                ...
                if stats.skipped_no_content <= MAX_LOGGED_WARNINGS:
                    _warn(
                        stats,
                        "У D2 нет описаний для content sync: sku=%s, supplier_code=%s",
                        mapping.sku,
                        mapping.supplier_code,
                    )

    logger.info(
        "Завершён D2 content sync: mapped=%d, joined=%d, inserted=%d, updated=%d",
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Синхронизация контента D2 из raw_supplier_feed_products в catalog_content"
    )
    parser.add_argument(
        "--limit",
        type=int,
        default=0,
        help="обработать только первые N mapping-строк D2 (0 = без лимита)",
    )
...
async def _amain() -> None:
    args = _parse_args()
    result = await sync_d2_content(limit=args.limit)
