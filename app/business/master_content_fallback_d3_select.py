...
logger = logging.getLogger("master_content_fallback_d3_select")
...
class FallbackStats:
    ...
    def to_dict(self) -> Dict[str, int]:
        return {
            ...
            "d3_ua_found": self.d3_ua_found,
            "d3_ru_found": self.d3_ru_found,
...
async def select_d3_fallback_content() -> Dict[str, int]:
    stats = FallbackStats()
    ...
    async with get_async_db() as session:
            ...
            await session.execute(
                ...
                .join(
                    ...
                    and_(
                        ...
                        CatalogContent.supplier_id == D3_SUPPLIER_ID,
        ...
        stats.d3_ua_found = sum(1 for _, language in selected_by_key.keys() if language == "ua")
        stats.d3_ru_found = sum(1 for _, language in selected_by_key.keys() if language == "ru")

        logger.info("Найдено D3 fallback descriptions ua: %d", stats.d3_ua_found)
        logger.info("Найдено D3 fallback descriptions ru: %d", stats.d3_ru_found)
...
async def _amain() -> None:
    result = await select_d3_fallback_content()
