...
logger = logging.getLogger("d10_content_sync")
...
async def _upsert_content(
    *,
    ...
    stmt = select(CatalogContent).where(
        ...
        CatalogContent.supplier_id == D10_SUPPLIER_ID,
...
async def sync_d10_content(limit: int = 0) -> Dict[str, Any]:
    stats = SyncStats()
    logger.info("Запуск D10 content sync для supplier_id=%s", D10_SUPPLIER_ID)
        ...
        for mapping in mapping_rows:
            ...
            if raw is None:
                ...
                if stats.skipped_no_raw <= MAX_LOGGED_WARNINGS:
                    _warn(
                        stats,
                        "Не найдена raw D10 запись для content sync: sku=%s, supplier_code=%s",
                        mapping.sku,
            ...
            if payload is not None and not isinstance(payload, dict):
                ...
                if stats.skipped_no_content <= MAX_LOGGED_WARNINGS:
                    _warn(
                        stats,
                        "Неожиданная структура source_payload у D10: sku=%s, supplier_code=%s",
                        mapping.sku,
                        mapping.supplier_code,
                    )
                continue

            title = _extract_payload_value(payload, "title") or _normalize_string(raw.name_raw)
            ...
            if not description:
                ...
                if stats.skipped_no_content <= MAX_LOGGED_WARNINGS:
                    _warn(
                        stats,
                        "У D10 нет описания для content sync: sku=%s, supplier_code=%s",
                        mapping.sku,
                        mapping.supplier_code,
                    )
                continue
    ...
    logger.info(
        "Завершён D10 content sync: mapped=%d, joined=%d, inserted=%d, updated=%d",
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Синхронизация контента D10 из raw_supplier_feed_products в catalog_content"
    )
    parser.add_argument(
        "--limit",
        type=int,
...
async def _amain() -> None:
    args = _parse_args()
    result = await sync_d10_content(limit=args.limit)
