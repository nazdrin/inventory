...
logger = logging.getLogger("d13_images_sync")
...
def _extract_images(source_payload: Any) -> List[str]:
    ...
    if not isinstance(raw_images, list):
        raw_images = source_payload.get("picture")
    if not isinstance(raw_images, list):
        return []
...
async def sync_d13_images(limit: int = 0) -> Dict[str, Any]:
    stats = SyncStats()
    logger.info("Запуск D13 images sync для supplier_id=%s", D13_SUPPLIER_ID)
        ...
        for mapping in mapping_rows:
            ...
            if raw is None:
                ...
                if stats.skipped_no_raw <= MAX_LOGGED_WARNINGS:
                    _warn(stats, "Не найдена raw D13 запись для images sync: sku=%s, supplier_code=%s", mapping.sku, mapping.supplier_code)
                continue
            ...
            if not images:
                ...
                if stats.skipped_no_images <= MAX_LOGGED_WARNINGS:
                    _warn(stats, "У D13 нет картинок в source_payload: sku=%s, supplier_code=%s", mapping.sku, mapping.supplier_code)
                continue
            ...
            for sort_order, image_url in enumerate(images):
                if _looks_strange_url(image_url) and stats.warnings_count < MAX_LOGGED_WARNINGS:
                    _warn(stats, "Подозрительный image URL для D13: sku=%s, supplier_code=%s, url=%s", mapping.sku, mapping.supplier_code, image_url)

                image_stmt = select(CatalogImage).where(
                    CatalogImage.sku == mapping.sku,
                    CatalogImage.supplier_id == D13_SUPPLIER_ID,
    ...
    logger.info(
        "Завершён D13 images sync: mapped=%d, joined=%d, inserted=%d, updated=%d",
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Синхронизация картинок D13 из raw_supplier_feed_products в catalog_images")
...
async def _amain() -> None:
    args = _parse_args()
    result = await sync_d13_images(limit=args.limit)
