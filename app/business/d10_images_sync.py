...
logger = logging.getLogger("d10_images_sync")
...
@dataclass
class SyncStats:
    supplier_id: int = D10_SUPPLIER_ID
...
def _extract_images(source_payload: Any) -> List[str]:
    ...
    main_image = _normalize_string(source_payload.get("image_link"))
    if main_image:
        seen.add(main_image)
        result.append(main_image)

    raw_additional = source_payload.get("additional_images")
...
async def sync_d10_images(limit: int = 0) -> Dict[str, Any]:
    stats = SyncStats()
    logger.info("Запуск D10 images sync для supplier_id=%s", D10_SUPPLIER_ID)

    async with get_async_db() as session:
        mapping_stmt = (
            select(CatalogSupplierMapping)
            .where(CatalogSupplierMapping.supplier_id == D10_SUPPLIER_ID)
        ...
        for mapping in mapping_rows:
            raw = (
                await session.execute(
                    select(RawSupplierFeedProduct).where(
                        RawSupplierFeedProduct.supplier_id == D10_SUPPLIER_ID,
            ...
            if raw is None:
                ...
                if stats.skipped_no_raw <= MAX_LOGGED_WARNINGS:
                    _warn(
                        stats,
                        "Не найдена raw D10 запись для images sync: sku=%s, supplier_code=%s",
                        mapping.sku,
            ...
            if not images:
                ...
                if stats.skipped_no_images <= MAX_LOGGED_WARNINGS:
                    _warn(
                        stats,
                        "У D10 нет картинок в source_payload: sku=%s, supplier_code=%s",
                        mapping.sku,
                        mapping.supplier_code,
                    )
                continue
            ...
            for sort_order, image_url in enumerate(images):
                if _looks_strange_url(image_url) and stats.warnings_count < MAX_LOGGED_WARNINGS:
                    _warn(
                        stats,
                        "Подозрительный image URL для D10: sku=%s, supplier_code=%s, url=%s",
                        mapping.sku,
                        mapping.supplier_code,
                        image_url,
                    )
                ...
                image_stmt = select(CatalogImage).where(
                    ...
                    CatalogImage.supplier_id == D10_SUPPLIER_ID,
    ...
    logger.info(
        "Завершён D10 images sync: mapped=%d, joined=%d, inserted=%d, updated=%d",
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Синхронизация картинок D10 из raw_supplier_feed_products в catalog_images"
    )
    parser.add_argument(
        "--limit",
        type=int,
        default=0,
        help="обработать только первые N mapping-строк D10 (0 = без лимита)",
    )
...
async def _amain() -> None:
    args = _parse_args()
    result = await sync_d10_images(limit=args.limit)
