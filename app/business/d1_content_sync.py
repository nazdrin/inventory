...
logger = logging.getLogger("d1_content_sync")
...
async def sync_d1_content(limit: int = 0) -> Dict[str, Any]:
    stats = SyncStats()
    logger.info("Запуск D1 content sync для supplier_id=%s", D1_SUPPLIER_ID)

    async with get_async_db() as session:
        mapping_stmt = (
            select(CatalogSupplierMapping)
            .where(CatalogSupplierMapping.supplier_id == D1_SUPPLIER_ID)
        ...
        for mapping in mapping_rows:
            raw = (
                await session.execute(
                    select(RawSupplierFeedProduct).where(
                        RawSupplierFeedProduct.supplier_id == D1_SUPPLIER_ID,
            ...
            if raw is None:
                ...
                if stats.skipped_no_raw <= MAX_LOGGED_WARNINGS:
                    _warn(
                        stats,
                        "Не найдена raw D1 запись для content sync: sku=%s, supplier_code=%s",
                        mapping.sku,
            ...
            if not isinstance(payload, dict):
                ...
                if stats.skipped_no_content <= MAX_LOGGED_WARNINGS:
                    _warn(
                        stats,
                        "Неожиданная структура source_payload у D1: sku=%s, supplier_code=%s",
                        mapping.sku,
                        mapping.supplier_code,
                    )
                continue
            ...
            if not has_content:
                ...
                if stats.skipped_no_content <= MAX_LOGGED_WARNINGS:
                    _warn(
                        stats,
                        "У D1 нет описаний для content sync: sku=%s, supplier_code=%s",
                        mapping.sku,
                        mapping.supplier_code,
                    )

    logger.info(
        "Завершён D1 content sync: mapped=%d, joined=%d, inserted=%d, updated=%d",
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Синхронизация контента D1 из raw_supplier_feed_products в catalog_content"
    )
    parser.add_argument(
        "--limit",
        type=int,
        default=0,
        help="обработать только первые N mapping-строк D1 (0 = без лимита)",
    )
...
async def _amain() -> None:
    args = _parse_args()
    result = await sync_d1_content(limit=args.limit)
