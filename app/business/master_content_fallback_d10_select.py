...
logger = logging.getLogger("master_content_fallback_d10_select")
...
class FallbackStats:
    ...
    def to_dict(self) -> Dict[str, int]:
        return {
            ...
            "d10_ua_found": self.d10_ua_found,
            "d10_ru_found": self.d10_ru_found,
...
async def select_d10_fallback_content() -> Dict[str, int]:
    stats = FallbackStats()
    ...
    async with get_async_db() as session:
        ...
        stats.d10_ua_found = sum(1 for _, language in selected_by_key.keys() if language == "ua")
        stats.d10_ru_found = sum(1 for _, language in selected_by_key.keys() if language == "ru")

        logger.info("Найдено D10 fallback descriptions ua: %d", stats.d10_ua_found)
        logger.info("Найдено D10 fallback descriptions ru: %d", stats.d10_ru_found)
...
async def _amain() -> None:
    result = await select_d10_fallback_content()
