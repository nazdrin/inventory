...
logger = logging.getLogger("d1_images_sync")
...
async def sync_d1_images(limit: int = 0) -> Dict[str, Any]:
    stats = SyncStats()
    logger.info("Запуск D1 images sync для supplier_id=%s", D1_SUPPLIER_ID)
        ...
        for mapping in mapping_rows:
            ...
            if raw is None:
                ...
                if stats.skipped_no_raw <= MAX_LOGGED_WARNINGS:
                    _warn(
                        stats,
                        "Не найдена raw D1 запись для images sync: sku=%s, supplier_code=%s",
                        mapping.sku,
            ...
            if not images:
                ...
                if stats.skipped_no_images <= MAX_LOGGED_WARNINGS:
                    _warn(
                        stats,
                        "У D1 нет картинок в source_payload: sku=%s, supplier_code=%s",
                        mapping.sku,
                        mapping.supplier_code,
                    )
                continue
            ...
            for sort_order, image_url in enumerate(images):
                if _looks_strange_url(image_url) and stats.warnings_count < MAX_LOGGED_WARNINGS:
                    _warn(
                        stats,
                        "Подозрительный image URL для D1: sku=%s, supplier_code=%s, url=%s",
                        mapping.sku,
                        mapping.supplier_code,
                        image_url,
                    )
                ...
                existing.is_main = sort_order == 0
                existing.source_type = SOURCE_TYPE
                stats.updated += 1

    logger.info(
        "Завершён D1 images sync: mapped=%d, joined=%d, inserted=%d, updated=%d",
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Синхронизация картинок D1 из raw_supplier_feed_products в catalog_images"
    )
    parser.add_argument(
        "--limit",
        type=int,
...
async def _amain() -> None:
    args = _parse_args()
    result = await sync_d1_images(limit=args.limit)
