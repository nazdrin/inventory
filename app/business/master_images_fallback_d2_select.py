...
logger = logging.getLogger("master_images_fallback_d2_select")
...
class FallbackStats:
    ...
    d2_images_found: int = 0
    updated_products: int = 0

    def to_dict(self) -> Dict[str, int]:
        return {
            "total_products_without_image": self.total_products_without_image,
            "d2_images_found": self.d2_images_found,
            "updated_products": self.updated_products,
        }


async def select_d2_fallback_main_images() -> Dict[str, int]:
    stats = FallbackStats()
    ...
    async with get_async_db() as session:
        ...
        stats.d2_images_found = len(selected_by_sku)
        logger.info("Найдено fallback-картинок D2: %d", stats.d2_images_found)
    ...
    logger.info("Обновлено товаров fallback-картинками D2: %d", stats.updated_products)
    return stats.to_dict()


async def _amain() -> None:
    result = await select_d2_fallback_main_images()
