...
logger = logging.getLogger("d3_content_sync")
...
async def sync_d3_content(limit: int = 0) -> Dict[str, Any]:
    stats = SyncStats()
    logger.info("Запуск D3 content sync для supplier_id=%s", D3_SUPPLIER_ID)
        ...
        for mapping in mapping_rows:
            ...
            if raw is None:
                ...
                if stats.skipped_no_raw <= MAX_LOGGED_WARNINGS:
                    _warn(
                        stats,
                        "Не найдена raw D3 запись для content sync: sku=%s, supplier_code=%s",
                        mapping.sku,
            ...
            if not isinstance(payload, dict):
                ...
                if stats.skipped_no_content <= MAX_LOGGED_WARNINGS:
                    _warn(
                        stats,
                        "Неожиданная структура source_payload у D3: sku=%s, supplier_code=%s",
                        mapping.sku,
                        mapping.supplier_code,
                    )
                continue
            ...
            if not has_content:
                ...
                if stats.skipped_no_content <= MAX_LOGGED_WARNINGS:
                    _warn(
                        stats,
                        "У D3 нет описаний для content sync: sku=%s, supplier_code=%s",
                        mapping.sku,
                        mapping.supplier_code,
                    )

    logger.info(
        "Завершён D3 content sync: mapped=%d, joined=%d, inserted=%d, updated=%d",
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Синхронизация контента D3 из raw_supplier_feed_products в catalog_content"
    )
    parser.add_argument(
        "--limit",
        type=int,
...
async def _amain() -> None:
    args = _parse_args()
    result = await sync_d3_content(limit=args.limit)
