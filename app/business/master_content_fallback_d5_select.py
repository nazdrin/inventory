...
logger = logging.getLogger("master_content_fallback_d5_select")
...
class FallbackStats:
    ...
    def to_dict(self) -> Dict[str, int]:
        return {
            ...
            "d5_ua_found": self.d5_ua_found,
            "d5_ru_found": self.d5_ru_found,
...
async def select_d5_fallback_content() -> Dict[str, int]:
    stats = FallbackStats()
    ...
    async with get_async_db() as session:
            ...
            await session.execute(
                ...
                .join(
                    ...
                    and_(
                        ...
                        CatalogContent.supplier_id == D5_SUPPLIER_ID,
        ...
        stats.d5_ua_found = sum(1 for _, language in selected_by_key.keys() if language == "ua")
        stats.d5_ru_found = sum(1 for _, language in selected_by_key.keys() if language == "ru")

        logger.info("Найдено D5 fallback descriptions ua: %d", stats.d5_ua_found)
        logger.info("Найдено D5 fallback descriptions ru: %d", stats.d5_ru_found)
...
async def _amain() -> None:
    result = await select_d5_fallback_content()
