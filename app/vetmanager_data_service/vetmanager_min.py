...
# берём твои существующие функции:
# - get_async_db, get_domain_and_key, discover_single_clinic, fetch_goods_paginated, vet_headers

async def discover_stores(domain: str, api_key: str, clinic_id: str, user_id: str,
                          sample_goods: List[str], timeout_sec: int = 20) -> Dict[str, str]:
    """
    Возвращает словарь {store_id: store_name} по первому встреченному названию.
    Берём по нескольким товарам, чтобы покрыть все активные склады.
    """
    base_stock = f"https://{domain}/rest/api/Good/StockBalancesForProduct"
    headers = {"X-REST-API-KEY": api_key, "Content-Type": "application/json", "X-REST-TIME-ZONE": "Europe/Kiev"}
    stores: Dict[str, str] = {}
    async with ClientSession(headers=headers, timeout=ClientTimeout(total=timeout_sec)) as s:
        for gid in sample_goods:
            url = f"{base_stock}?clinic_id={clinic_id}&good_id={gid}&user_id={user_id}"
            try:
                ...
                continue
            balances = (data.get("data") or {}).get("stock_balances") or []
            for row in balances or []:
                sid = str(row.get("store_id") or (row.get("store") or {}).get("id") or "")
                sname = ""
                store_node = row.get("store") or {}
                # пробуем разные поля названия
                for key in ("title", "name", "store_name"):
                    if store_node.get(key):
                        sname = str(store_node[key])
                        break
                if sid:
                    stores.setdefault(sid, sname)
    return stores

async def cli_list_stores(enterprise_code: str) -> None:
    # берём домен/ключ, клинику и первые N товаров
    async with get_async_db() as sess:
        domain, api_key = await get_domain_and_key(sess, enterprise_code)
    uid, clinic_id = discover_single_clinic(domain, api_key, get_logger("discover"))
    goods = fetch_goods_paginated(domain, api_key, get_logger("discover"), limit=100)
    sample = [str(g["id"]) for g in goods[:50] if g.get("id") is not None]  # до 50 шт. достаточно
    stores = await discover_stores(domain, api_key, clinic_id, uid, sample)
    if not stores:
        print("Склады не найдены (нет остатков/доступа).")
        return
    print("Найдены склады (store_id → name):")
    for sid, name in stores.items():
        print(f"  {sid:>6}  {name or '(без названия)'}")
