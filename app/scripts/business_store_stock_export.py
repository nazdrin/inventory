...
from app.business.business_store_stock_exporter import export_business_store_stock_by_selector
from app.database import get_async_db


def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Manual Business Store stock export for a single store.")
...
async def _amain() -> None:
    ...
    async with get_async_db(commit_on_exit=False) as session:
        result = await export_business_store_stock_by_selector(
            session,
